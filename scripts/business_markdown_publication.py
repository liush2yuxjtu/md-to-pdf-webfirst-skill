...
<section class="page"><p class="fig-label">图 2</p><h2>Fem、Shave 是当前正向驱动力，但不足以抵消三大问题品类压力。</h2><div class="big-layout"><div class="big-num"><b>{len(growth_categories)}</b><small>增长/健康品类</small></div><div class="flow"><div>Fem</div><div>Shave</div><div>Fabric</div><div>Hair / Oral</div></div><div class="big-num red"><b>{len(issue_categories)}</b><small>重点问题品类</small></div></div><div class="issue-grid">{praise_cards}</div><footer class="folio"><span>YANGFAN BUSINESS OVERVIEW</span><span>正向驱动</span><span>06</span></footer></section>
...
<section class="page"><p class="eyebrow">建议</p><h2>下一步不是泛泛“关注”，而是进入 Fabric、Hair、Oral 的 L2 下钻。</h2><div class="figure-card">{action_cards}</div><div class="note-grid">{action_gates}</div><p class="lead">尤其需要解释：Fabric + Hair 的 { _esc(decline_focus) } 下跌贡献，以及 Oral 环比暴跌 { _esc(oral_drop) } 背后的断货、渠道调整或数据异常。</p><footer class="folio"><span>YANGFAN BUSINESS OVERVIEW</span><span>行动建议</span><span>09</span></footer></section>
<section class="page"><p class="eyebrow">方法与来源</p><h2>本 PDF 保留源 Markdown 口径，并将表格与段落重构为出版级阅读路径。</h2><ol><li>源文件：{_esc(source_label)}</li><li>指标口径：IYA、PS 门店、同比、环比、覆盖进度均沿用源 Markdown。</li><li>重绘说明：品类压力图基于源段落提取 IYA，PS 表保留源表结构。</li></ol><footer class="folio"><span>YANGFAN BUSINESS OVERVIEW</span><span>来源</span><span>10</span></footer></section>
</body></html>"""
    meta = {
        "title": title,
        "source": source_label,
