...
- Treat the local McKinsey/MGI reference PDF as a positive craft benchmark, not a template to clone. Publication outputs need a topic-relevant human/portrait or editorial visual on the cover, reader-facing title/subtitle/date, an author or institution line, deliberate whitespace, designed table of contents, one-page summary, chapter-opener visuals, figure-numbered exhibits, footnotes/source notes, evidence/cases, and related-publication/reference pages when available. For generated business reports, use `Win-Channel AI Research Institute` as the default institution label unless the source or user specifies another publisher.
- Do not let sparse data stay sparse. When a page has only a handful of numbers, turn them into publication exhibits: big-number callouts, left-margin key metrics, unit-block share displays, before/after or current/future frames, bubble/circle matrices for ordinal categories, pie/donut charts for part-to-whole data, and map bubbles for geography.
